...
ATOM CWndMain::InitClass() {
	...
	wc.lpszMenuName = 0;
	wc.lpszClassName = L"CNtgWndMain";
	return RegisterClass(&wc);
}

bool CWndMain::Show() {
	HWND h = CreateWindow(L"CNtgWndMain", L"Nitrogen", WS_VISIBLE, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, player()->hInstance, NULL);
	if (h == NULL) {
		return false;
    }
	if (hWndMenuBar) {
...
void CWndMain::SetLSoftkeyText() {
	...
	SetSoftkeyText(hWndMenuBar, 0, _str(lSoft[player()->lpConfig->cf.LeftSoftkeyAction]));
}
	...
	switch (tmrId) {
		...
		case TMR_SLEEP:
			...
			if (player()->lpDlgSleep->SleepMins <= 0) {
				...
				if (player()->lpConfig->cf.SleepCloseApp) {
					SendMessage(hWnd, WM_CLOSE, 0, 0);
				} else {
					player()->StopAndClose();
				}
...
void CWndMain::StartMenuNames() {
	...
	SetWbrItem(&MenuMain.Data[4], _str(STR_SETTINGS), ICO_SETTINGS, false, false, true);
	SetWbrItem(&MenuMain.Data[5], _str(STR_ABOUT), ICO_ABOUT, false, true, false);
	SetWbrItem(&MenuMain.Data[6], _str(STR_BLANKSCREEN), ICO_BLANKSCREEN, false, false, false);
	SetWbrItem(&MenuMain.Data[7], _str(STR_EXIT), ICO_EXIT, false, false, false);

}

bool CWndMain::OnCreate() {

	MenuSettings.Count = 7;
	MenuSettings.Data = new WBRITEM[7];

	MenuMain.Count = 8;
	MenuMain.Data = new WBRITEM[8];
...
LRESULT CWndMain::WndProc(UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	switch (uMsg) {
		...
		case WM_COMMAND: {
			...
			if (wmId == IDM_LSOFTKEY) {		 OnSoftkeyClick(0);		}
			else if (wmId == IDM_OPTIONS) {	 OnSoftkeyClick(1);		}
