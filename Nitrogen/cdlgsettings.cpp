	...
	switch (uMsg) {
		...
		case WM_INITDIALOG: {
			...
			if (player()->lpConfig->cf.SleepCloseApp) {
				SendMessage(hCbbSleep, CB_SETCURSEL, 1, 0);
			} else {
				SendMessage(hCbbSleep, CB_SETCURSEL, 0, 0);
			}
			...
			bool chk[6] = {player()->lpConfig->cf.ForceCleartype, player()->lpConfig->cf.LockHardKeys, player()->lpConfig->cf.ChangeSongButtons, player()->lpConfig->cf.Id3Tags, player()->lpConfig->cf.RememberPos, player()->lpConfig->cf.WriteSongRegistry};
			int str[6]  = {STR_FORCECLEARTYPERE, STR_LOCKHARDWAREKEYS, STR_CHANGEPLAYINGSON, STR_READIDTAGSWHENAV, STR_REMEMBERLASTSONG, STR_WRITECURRENTSONG};
		...
		case WM_VSCROLL: {
 		...
 		case WM_COMMAND: {
			...
			if ((wmId == IDM_OK) | (wmId == IDOK)) {
				...
				player()->lpConfig->cf.SleepCloseApp = (cbSleepSel == 1);
