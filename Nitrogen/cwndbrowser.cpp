...
void CWndBrowser::OnCreate() {
	...
	SetSoftkeyText(hWndMenuBar, 0, lpSoft[0]);
	SetSoftkeyText(hWndMenuBar, 1, lpSoft[1]);
...
void CWndBrowser::SetSoftkey(int nSoft, LPWSTR newText) {
	...
	if (!ShowingMenu) {
		SetSoftkeyText(hWndMenuBar, nSoft, lpSoft[nSoft]);
	}
}
...
void CWndBrowser::StartMenu(WBRLIST * lMenu) {
	...
	ShowingMenu = true;
	SetSoftkeyText(hWndMenuBar, 0, L"Cancel");
	SetSoftkeyText(hWndMenuBar,	1, L"OK");
	Repaint();
}
