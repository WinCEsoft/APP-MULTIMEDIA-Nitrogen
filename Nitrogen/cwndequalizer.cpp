...
void CWndEqualizer::OnCreate() {
    ...
    mbi.hwndParent = hWnd;
    mbi.nToolBarId = MNU_WNDEQUALIZER;
    mbi.hInstRes = player()->hInstance;
    if (!SHCreateMenuBar(&mbi)) {
        hWndMenuBar = NULL;
    } else {
        hWndMenuBar = mbi.hwndMB;
		SetSoftkeyText(hWndMenuBar, 0, _str(STR_PRESETS));
...
void CWndEqualizer::OnMouseDown(int px, int py) {
	...
	} else if (iPointInRect(px, py, getRect(hBoxRect.left, hBoxRect.bottom+3, (int)(hBoxRect.left+fHeight*1.5), (int)(hBoxRect.bottom+3+fHeight*1.5)))) {
...
void CWndEqualizer::OnPaint(HDC hDC) {
	...
	if ((hBoxRect.top == 0) & (hBoxRect.bottom == 0)) {
		...
		hBoxRect.bottom = (int)min(get_devHei()-fHeight*1.5-10, fHeight*2+get_devWid()*0.7);
	}
	...
	DrawText(hBufDC, _str(STR_GRAPHICEQUALIZER), wcslen(_str(STR_GRAPHICEQUALIZER)), getRect(2, 2, get_devWid()-2, (int)(fHeight*1.5)), DT_LEFT|DT_BOTTOM);
	...
	iDrawCheckbox(hBufDC, hBoxRect.left, hBoxRect.bottom+3, get_devWid(), (int)(fHeight*1.5), _str(STR_ENABLEEQUALIZER), (eq.fEnable == TRUE));
...
int CWndEqualizer::MenuProc(int uMsg) {
	...
	if (uMsg == WBR_MENUSELECTED) {
		...
		// 1:Remove preset
		if (Menu.ItemIndex == 1) {
			Menu.Data[1].Checked = !Menu.Data[1].Checked;
		}
...
void CWndEqualizer::StartMenu() {
	...
	ClearWbrList(&Menu);
	Menu.Count = 3+presetCount;
	Menu.Data = new WBRITEM[3+presetCount];
	Menu.Data[0] = MenuPresetAdd;
	Menu.Data[1] = MenuPresetDel;
	Menu.Data[2] = MenuPresetClr;
	for (int i = 0; i < presetCount; i++) {
		SetWbrItem(&Menu.Data[3+i], preset[i].name, ICO_PRESET, false, false, false);
	}
